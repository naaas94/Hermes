...
**Version:** 0.5 · **Status:** Complete · **Source:** `.dev/evaluation-and-health-metrics-roadmap.md` Part B (lines 75–143)

> **v0.3 milestone (2026-04-16):** Part B declared complete. All six subtasks (plus the T1.1 follow-up) shipped. Contract at schema `"2.0"` with the accepted WARN-string deviation for `bench.dualsink.regression`. T2 KC3 is closed-with-evidence on the reference machine and remains runtime-armed via T4's dual-run gate. No kill criteria fired in the final state; two fired during execution and were resolved (T1 KC2 → T1.1; T2 KC3 → routed to T4 → closed with measurement). No non-goals were crossed.
>
> **v0.4 update (2026-04-17) — audit-driven amendments.** Auditor Part I returned **fail** with two `major` findings. Both verified against code: **F1 (`contract-violation`)** — `ObservabilityConfig` lacked typed `rss_sampling_*` fields; user TOML silently dropped. **F2 (`intent-drift`)** — plan + `packets/T6.md` cited non-existent `--workload small` flag; CI runs `hermes bench --mock-llm --output benchmarks`. Resolution: new subtask **T7** (F1 code fix + F5 README consumer-guidance paragraph); §4 T6 and `packets/T6.md` amended to match shipped CLI (F2 closed by amendment); `packets/T2.md` Outputs row amended (F6 closed by amendment). Status moved `Complete → Active`.
>
> **v0.5 milestone (2026-04-17) — audit rerun `pass-with-conditions`.** Auditor Part II (rerun after T7 shipped) confirmed: **F1 closed in code** (`ObservabilityConfig` has all five typed fields; `tests/test_obs_config.py` round-trips user TOML; `config.toml.example` fully specified), **F2 closed by amendment**, **F5** closed (README "Programmatic consumers" paragraph), **F6** closed (T2 packet amended). Pytest **206 passed**. New minor finding **F8 (`contract-violation`, documentation-only)**: the v0.4 `b-plan.md` §2 Config-fields row still carried my forward-looking "T7 outstanding" annotation — stale after T7 shipped. **Fixed in this revision** (the §2 row now describes post-T7 reality; F8 closed by v0.5 amendment). Remaining non-blocking follow-ups per auditor §9 conditions 2 & 3: **F3** (`decisions/T4.md` "Deferred" section still lists T5/T6 items as open; should be closed or rewritten by the T4 decision-log author) and **F4** (`decisions/T1.md` preamble "Alternatives rejected" contradicts the v0.2 contract-refresh section below it; should be trimmed or marked historical by the T1 decision-log author). Observations **F9** (CHANGELOG T2 bullet narrative incompleteness) and **F7** (cosmetic stage-name prose residue in frozen packets) remain logged but not actioned, per the auditor's "observation" designation. Status returned to **Complete**. Runtime-armed safety nets from v0.3 remain in force (dual-sink overhead gate, stage-vocabulary assumption, CI artifact stability).
...
| Config fields | `hermes/config.py` | All five observability fields are **typed on `ObservabilityConfig`** (frozen dataclass at `config.py:60-68`) and routed through `_parse_config`'s `obs_fields = {k: v for k, v in obs_raw.items() if k in ObservabilityConfig.__dataclass_fields__}` filter, so user TOML values propagate to the runtime config object. Fields and defaults: `log_format: str = "console"` (values: `"json"` or `"console"`), `log_dir: str = "logs"`, `log_ndjson: bool = False` *(additive in v0.2.1 per T2 decision log — independent NDJSON toggle)*, `rss_sampling_enabled: bool = True`, `rss_sampling_interval_s: float = 0.0` *(0 = boundaries-only; >0 = periodic sampler cadence)*. Pipeline reads the two RSS fields via typed attribute access (`cfg.observability.rss_sampling_enabled` / `cfg.observability.rss_sampling_interval_s`) at `hermes/extraction/pipeline.py:246-629`; the pre-v0.5 defensive `getattr(cfg.observability, ..., default)` idiom has been removed. **v0.5 status:** audit F1 (Part I) closed in code by **T7**; `tests/test_obs_config.py` guards the round-trip from user TOML to `ObservabilityConfig`; `config.toml.example` documents the full `[observability]` block. Audit F8 (Part II rerun) — the pre-v0.5 version of this row still carried forward-looking "T7 outstanding" language and is itself what F8 flagged as documentation drift; fixed in this revision. |
...
graph LR
    ...
    T5 --> T7
    T6 --> T7
    T7["T7: Audit fix-up<br/>(config contract + README)<br/><i>post-v0.3</i>"]
```

**Parallel groups (v0.4):**
...
- `{T7}` — **post-v0.3 amendment** added by v0.4. Depends on T4 (CLI surface stable) and T5 (README structure to append to). Not a blocker for any existing subtask; closes F1 + F5 from the 2026-04-17 audit.
...
- T7 → T5 is soft: T7's README paragraph appends to the "Benchmarks & memory" section T5 added; if T5 ever renames the section, T7 follows.
...
|-------|---------|
| **ID** | T6 |
| **Scope** | Add a step to `.github/workflows/ci.yml` that, on pushes to `main` only, runs the bench in informational mode and uploads the resulting JSON as a GitHub Actions artifact. **v0.4 amendment (F2):** the invocation as shipped is `hermes bench --mock-llm --output benchmarks` (writing to the `benchmarks/` directory with auto-named `<date>_<sha>.json` per T4 contract), followed by `actions/upload-artifact@v4` globbing `benchmarks/*.json` into `bench-summary-${{ github.sha }}`. The pre-v0.4 plan text specified `--workload small --output benchmarks/ci-<run-id>.json`; T4 did not ship a `--workload` flag (it uses per-workload `compare_log_format` defaults + an `--include-large` boolean instead), so CI selects the default workload set rather than filtering. The bench step runs under `continue-on-error: true` with `timeout-minutes: 5`; the upload step is gated by `steps.bench.outcome != 'skipped'` so a skipped bench does not produce a failed upload. Explicitly do **not** add a regression gate (per Part B non-goals). |
...
| **Contract bindings** | CLI flag names must match T4 verbatim. **v0.4 amendment (F2):** the authoritative T4 CLI surface is `--mock-llm`, `--output`, `--include-large`, `--log-format-compare/--no-log-format-compare`, `--model`, `--workers`. There is **no** `--workload` flag; workload selection is controlled by `--include-large` (add the large PDF to the default set) and per-workload `compare_log_format` defaults inside `hermes/bench/workloads.py`. Artifact naming convention `bench-summary-${{ github.sha }}` documented in T5 and the CI workflow. |
...
| **Log tier** | standard |
| **Risks & mitigations** | **Risk:** CI runner RSS numbers are useless because of shared tenancy and caching. **Mitigation:** Document this in T5's methodology; the CI artifact's purpose is "this keeps working and produces a parseable summary", not "this is a regression oracle". **Risk:** Adding a job slows CI. **Mitigation:** Scope to `main` branch only (not PRs); rely on `continue-on-error: true` + `timeout-minutes: 5`; fail-soft on timeout. |

---

### T7 — Audit fix-up: typed config fields + README consumer guidance (post-v0.3)

| Field | Content |
|-------|---------|
| **ID** | T7 |
| **Scope** | Close two findings from the 2026-04-17 audit without disturbing the v2.0 schema or any shipped behavior. (a) **F1 fix:** add `rss_sampling_enabled: bool = True` and `rss_sampling_interval_s: float = 0.0` as typed fields on `hermes.config.ObservabilityConfig`, so `_parse_config`'s `obs_fields = {k: v for k, v in obs_raw.items() if k in ObservabilityConfig.__dataclass_fields__}` actually routes these through from user TOML. Update `hermes/extraction/pipeline.py` to read via direct attribute access (dropping the defensive `getattr(..., default)` idiom) so the typed contract is the single source of truth. Add a `[observability]` block to `config.toml.example` showing all five fields with their defaults. Add a test in `tests/test_obs_logging.py` or a new `tests/test_obs_config.py` that round-trips `rss_sampling_enabled=false` and `rss_sampling_interval_s=0.25` from TOML → `HermesConfig` and asserts the values arrive on `ObservabilityConfig`. (b) **F5 fix:** append a short "Programmatic consumers" paragraph to the README's "Benchmarks & memory" section restating that `bench.dualsink.regression` is emitted as a WARN-level log line with that literal message string, **not** as a structured `EventName`, and that consumers detecting regressions should filter log records on `level == "WARNING"` + message substring, not on `event` equality. Cross-reference `b-plan.md` §2 and `decisions/T4.md`. |
| **Files to touch** | Code + tests: `hermes/config.py` (add two dataclass fields), `hermes/extraction/pipeline.py` (replace `getattr(cfg.observability, "rss_sampling_enabled", True)` and `getattr(..., "rss_sampling_interval_s", 0)` with typed attribute access — discovery required: grep for both strings to catch every call site), `config.toml.example` (add or extend `[observability]` block — discovery required: confirm file exists at repo root; if absent, create it), `tests/test_obs_config.py` (new — 1–2 round-trip tests) OR extend `tests/test_obs_logging.py` (executor's choice; new file preferred so config tests are not mixed with logger tests). Docs: `README.md` ("Benchmarks & memory" section — append the "Programmatic consumers" paragraph; discovery required: confirm section heading and do not restructure surrounding content). Audit hygiene (optional but recommended): `.dev/audits/2026-04-17-part-b-b-plan.md` (append a "2026-XX-XX resolution" block noting F1 + F5 closed by T7; do not edit the auditor's original findings). |
| **Contract bindings** | Shared contracts as of v0.4. `ObservabilityConfig` gains two additive fields (no breaking change; no `LogSchemaVersion` bump because `LogSchemaVersion` tracks NDJSON event shape, not config shape). No changes to `StageName`, `EventName`, `BaseEvent`, `run_type`, or the error envelope. README additions must match the `b-plan.md` §2 `EventName`-row consumer guidance verbatim (copy the two sentences about WARNING-level + substring). |
| **Inputs** | T4 (CLI surface stable), T5 (README section "Benchmarks & memory" exists to append to), T6 (CI workflow informs the `hermes bench` CI command referenced in the paragraph if helpful). |
| **Outputs** | Two new typed config fields; `config.toml.example` with a fully-specified `[observability]` block; at least one round-trip TOML test asserting user values propagate to `ObservabilityConfig`; README paragraph giving programmatic consumer guidance for `bench.dualsink.regression`; pytest + ruff + mypy all green. |
| **Kill criteria** | HALT if: (1) Adding the two fields to `ObservabilityConfig` breaks existing tests that compare config objects by exact field set (e.g. a test asserting `ObservabilityConfig.__dataclass_fields__.keys() == {"log_format", "log_dir", "log_ndjson"}`) — update the test; if the failure hides a deliberate invariant, escalate before changing the assertion. (2) `config.toml.example` does not exist at the repo root and the repo uses a different example-config convention (e.g. docs-only) — flag the ambiguity rather than creating a stray file. (3) `hermes/extraction/pipeline.py` reads one of the two fields inside a hot loop and dropping `getattr` changes observable behavior (e.g. silently re-enables the periodic sampler on machines where it was defaulted off by `getattr` fallback) — verify equivalence on a manual `hermes bench` run before merging; if behavior changes, leave the `getattr` call site alone and only add the typed field. (4) The README "Benchmarks & memory" section has been restructured since T5 and no longer has a natural home for the new paragraph — insert at the closest subheading and flag the reshape in the executor's changelog note, but do not reshape the section itself. |
| **Log tier** | standard |
| **Risks & mitigations** | **Risk:** Removing `getattr` defaults changes behavior if another code path set the fields to non-dataclass-valid values. **Mitigation:** Keep defaults identical (`True` and `0.0`); run the full pipeline test suite and `hermes bench --mock-llm` locally before merging. **Risk:** The new TOML test is brittle across Python / tomllib versions. **Mitigation:** Use a string-literal TOML fixture inside the test; do not read a committed file. **Risk:** The README paragraph drifts from `b-plan.md` §2 over time. **Mitigation:** Quote the two sentences verbatim from §2 and link back to the plan + `decisions/T4.md`; a future README rewrite should respect the verbatim-cite constraint. |
...
- **T5 ↔ T4 CLI flags.** Docs reference the shipped T4 flag surface (`--mock-llm`, `--output`, `--include-large`, `--log-format-compare/--no-log-format-compare`, `--model`, `--workers`). If T4 renames any of these, T5 must update. Mitigation: T6 and T5 both cite the flag names via the shared-contracts section; if T4 renames, it must update shared contracts first, which is a visible change. **v0.4 note (F2):** the original v0.1 planning text assumed a `--workload` filter flag; T4 did not ship one and instead expressed workload selection via `--include-large` and per-workload defaults. The v0.4 amendment corrects plan + packet T6 to the shipped surface; future edits should cite the surface above, not the pre-v0.4 text.
...
- [T2 — Structured logger with dual sink and config flag](packets/T2.md) *(shipped; KC1+KC2 resolved inline, KC3 closed-with-evidence via T4; **v0.4 packet amendment:** Outputs row updated per audit F6 to remove pre-v0.2 stdlib-fallback language)*
...
- [T6 — CI artifact upload for bench summary JSON (main-branch runs)](packets/T6.md) *(shipped; **v0.4 packet amendment:** scope + contract-bindings + kill-criteria rows updated per audit F2 to match shipped CLI — no `--workload` flag)*
- [T7 — Audit fix-up: typed config fields + README consumer guidance](packets/T7.md) *(shipped in v0.5; audit rerun confirms F1 + F5 closed in code)*
...
- **0.5 (2026-04-17):** Audit rerun (Part II of `.dev/audits/2026-04-17-part-b-b-plan.md`) returned **`pass-with-conditions`**. T7 shipped successfully: `ObservabilityConfig` has all five typed fields, `tests/test_obs_config.py` round-trips user TOML, `config.toml.example` documents the full `[observability]` block, `pipeline.py` reads RSS fields via typed attribute access (no `getattr` defaults), README "Programmatic consumers" paragraph in place. Pytest **206 passed**. Part I majors F1 + F2 fully closed; Part I minors F5 + F6 fully closed. New Part II finding **F8** (minor `contract-violation`, documentation-only) — `b-plan.md` §2 Config-fields row still carried the v0.4 forward-looking "T7 outstanding" annotation, stale after T7 shipped. **Fixed in this revision:** §2 Config-fields row now describes the post-T7 reality (five typed fields, typed pipeline access, F1 closed-in-code). Status returned `Active → Complete` per auditor condition 1. Remaining auditor conditions 2 and 3 (non-blocking for code merge) are logged as tracked **executor-owned follow-ups**: **F3** — `decisions/T4.md` "Deferred" section still lists T5/T6 items as open (owner: T4 decision-log author); should be rewritten as "Completed (T5: README reference row in place; T6: CI artifact upload + `ci.yml` bench step shipped)". **F4** — `decisions/T1.md` preamble "Alternatives rejected" contradicts the v0.2 contract-refresh section below it (owner: T1 decision-log author); should be trimmed or labeled historical to remove the contradiction with Option B. **F9** (observation) — `CHANGELOG.md` T2 entry lists only three `ObservabilityConfig` fields, corrected by the T7 entry immediately below it; narrative redundancy only, no action. **F7** (observation) — cosmetic `normalize`/`chunk`/`extract` prose residue in frozen packets (T3/T4 task-statement lines); canonical post-v0.2 spelling is `normalization`/`chunking`/`extraction`; no action per auditor designation. No code changes in this revision. Runtime-armed safety nets from v0.3 remain in force.
- **0.4 (2026-04-17):** Audit-driven amendments in response to `.dev/audits/2026-04-17-part-b-b-plan.md` (verdict: **fail** with pass-with-conditions path). Orchestrator verified both `major` findings against code. **F1 (`contract-violation`):** `ObservabilityConfig` missing typed `rss_sampling_enabled` + `rss_sampling_interval_s`; user TOML silently dropped. New subtask **T7** added to close F1 (code + test + `config.toml.example` + README paragraph also closing F5). **F2 (`intent-drift`):** plan + `packets/T6.md` cited a non-existent `--workload small` flag; fixed in-place — §4 T6 scope, contract bindings, and kill-criteria rows now describe the shipped CLI (`--mock-llm`, `--output`, `--include-large`, `--log-format-compare`); packets/T6.md amended to match; residual `--workload` references in §4 T6 Contract bindings, rejected-alternative D, and hidden coupling "T5 ↔ T4 CLI flags" scrubbed. **F6 (`process-violation`):** `packets/T2.md` Outputs row still carried pre-v0.2 "structlog missing → stdlib fallback" language; corrected inline (T1.1 cascade was row-scoped and missed this). DAG extended with `T5 --> T7` and `T6 --> T7` edges; T7 is a post-v0.3 amendment, not a blocker for anything existing. `ObservabilityConfig` field list in §2 amended to document the current reality (three typed fields) + T7 outstanding (two more). Status moved `Complete → Active (audit amendments in flight)`; returns to `Complete` when T7 ships and auditor reruns. **Tracked executor-owned follow-ups** (not blocking v0.4 transition to pass-with-conditions): F3 — `decisions/T4.md` "Deferred" section still lists T5/T6 items as open; should be closed by T4 decision-log author. F4 — `decisions/T1.md` retains pre-v0.2 stage-vocabulary "Alternatives rejected"; should be reconciled by T1 decision-log author. F7 — cosmetic `normalize`/`chunk`/`extract` prose residue in frozen packets; `normalization`/`chunking`/`extraction` is the post-v0.2 canonical spelling. None of the three affect correctness; they are log hygiene. Non-goals unchanged.
